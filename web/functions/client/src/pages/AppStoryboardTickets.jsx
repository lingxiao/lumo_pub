 ...
 *
**/
export default function AppStoryboardTickets(props){
	...
	function CustomInviteView(){
		...
		return (
			<AboutRareText 
				name={name} 
				about={about} 
				h1={`${top}/${num_editions} tickets sold`}
				h2={`* Price: ${price} ETH`}
			/>				
		)
	}
	...
	return (
		<div className={tclasses.scroll_container}>
			...
			/>			
			<TicketHolderTitle next_page_of_users={next_page_of_users} container_style={{width:'85%'}} data={tix_users} show_next_btn />
				...
				get_current_datasource().map((item,index) => {
					return (
						<UserRowBlock 
							...
							h2 = {`Purchased on ${item.ppTimeStamp ?? ""}`}
							row_style={{}}
						/>
					)					
				})
