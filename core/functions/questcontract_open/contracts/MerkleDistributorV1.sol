...
// author: lingxiao
// date: Dec 14 2022
// module: Generalized quest function that works with `TestNFT.sol`
...
contract MerkleDistributorV1 {
    ...
    */
    ...
    /** 
     * @dev send `faucetAmtPerDrip` to `tgt` so that it can sign tx.
     **/
    function dripFaucet() public {
        address tgt = msg.sender;
        require(tgt != address(0), "Target cannot be zero address");
        require(faucetTargets[tgt] < faucetLimitTotal, "You have exceeded the limit");
        require(faucetBalance > faucetAmtPerDrip, "Faucet is dry");
        Address.sendValue(payable(tgt),faucetAmtPerDrip);
        faucetTargets[tgt] += faucetAmtPerDrip;
        faucetBalance -= faucetAmtPerDrip;
    }
    ...
    function readFaucet() public view returns (uint256){
        return faucetBalance;
    }

    function resetFauct(address tgt) public {
        require(msg.sender == contractOwner, "Only contract owner can reset faucet");
        faucetTargets[tgt] = 0;
    }
    ...
    */
    ...
    /**
     * @dev create quest where by winners receive erc721 token in airdrop
     **/
    function createERC721Quest(
        uint256 _id,
        uint _maxWinners,
        bytes32 _merkleRoot,
        ...
        quests[_id] = Quest(_id, msg.sender, _maxWinners, _merkleRoot, _tokenAddress, loot, msg.value, _metadataURI,InterfaceKind.ERC721);
    ...
    function createERC20Quest(
        ...
        require(_maxWinners > 0, "Max winners must be > 0");
        require(claimedQuestIds[_id] == 0, "This quest id has already been claimed");        
        require(msg.value > 0, "Quest rewards cannot both be 0");
        questIds.push(_id);
        claimedQuestIds[_id] = 1;
        uint256 loot = msg.value/_maxWinners;
        quests[_id] = Quest(_id, msg.sender, _maxWinners, _merkleRoot, _tokenAddress, loot, msg.value, _metadataURI,InterfaceKind.ERC20);
        questRewardBalance[_id] = msg.value;
        emit QuestCreated(_id, msg.sender, _tokenAddress);
    }
    ...
    **/
     ...
     *      if the quest reward is `loot`, then airdrop loot from this contract address, return `tok_id=0` in this case;
     **/
    function claimQuest( uint256 _questId, bytes32[] calldata merkleProof ) public returns (uint256) {
        ...
        } else if (q.face == InterfaceKind.ERC721) {

            uint256 tok_id = TestNFT(q.tokenAddress).mintNFT(msg.sender);
            questWinners[_questId][msg.sender] = 1;
            questWinningProofs[_questId][merkleProofInStr] = 1;
